...
- **Runtime credential/config auto-detection**: formulae that need to discover user credentials at *service start time* (not install time) do so from the launcher script itself, because Homebrew's build sandbox denies filesystem reads outside a fixed allowlist during `install`/`post_install_steps` — see the kiro-gateway launcher script and its `post_install_steps` `.env` handling.
